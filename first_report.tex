...
\n In our design, both \textit{scanner} and \textit{parser} will be able to
...
characters or it cannot be classified as any \emph{symbol\_type} (e.g. a number
...
the scanner does not match the expected \emph{symbol\_type} specified by the
language rules.

\vspace{0.3cm}

\n The \textit{parser} will identify the semantic errors as well: if the parser can
...
\emph{error symbol\_type} and an \emph{error symbol\_ID} to the parser. The
...
encountered, the \textit{error\_display} function will be called. It will get
the error location via the two variables from the scanner and pass the error
information to \textit{error\_display}. Another variable \emph{error\_count} in
the class \textit{parser} will record the total number of errors.
