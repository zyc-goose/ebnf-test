...
\n We also defined whitespace characters, which include normal space,
newline(\texttt{'$\backslash$n'}) and tab(\texttt{'$\backslash$t'}). Since spacing
is used to separate the keywords, numbers and identifiers in the definition
file, we have defined a non-terminal variable \texttt{space} which contains at
least one whitespace character. \texttt{space} plays a crucial part in the
language grammar.
